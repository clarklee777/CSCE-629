...
public:
    ...
    {
        /* Bug is the vertex's index in the heap structure, not the vertex number, for H[] */
        /* When the bug is larger than its parent node : push up swap */
        if((bug>1)&&(D[H[bug]]>D[H[bug/2]]))
        {
            int temp = bug;
            
            /* Keeping swapping while the bug's data is still larger than its parent's data */
            while ((temp>1)&&(D[H[temp]]>D[H[temp/2]]))
            {
                int h_temp = H[temp];
                int d_temp = D[H[temp]];
        ...
        else if ((bug<=(h_size/2))&&((D[H[bug]]<D[H[bug*2]])||(D[H[bug]]<D[H[bug*2 + 1]])))
        {
            int temp = bug;
            
            /* Keeping swapping while the bug's child is still larger than itself */
            while ((temp<=(h_size/2))&&((D[H[temp]]<D[H[temp*2]])||(D[H[temp]]<D[H[temp*2 + 1]])))
            {
                int child_for_swap = temp*2;
                int max_value = D[H[temp*2]];
                
                if(max_value<D[H[temp*2 + 1]])
                {
                    max_value = D[H[temp*2 + 1]];
                ...
                D[H[temp]] = D[H[child_for_swap]];
                
                H[child_for_swap] = h_temp;
                D[H[child_for_swap]] = d_temp;
